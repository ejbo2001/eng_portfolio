...
const lucideIconSchema = z
	.object({
		...
		name: z.custom<keyof typeof lucideIcons>(
			...
			"El nombre del ícono Lucide no puede estar vacío",
		),
	})
...
const simpleIconSchema = z
	.object({
		...
		name: z.custom<keyof typeof simpleIcons>(
			(val) => typeof val === "string" && val.length > 0,
			"El nombre del ícono Simple no puede estar vacío",
		),
	})
	.strict();
...
const socials = defineCollection({
	...
	schema: z
		.object({
			...
			link: z
				.string()
				.url("[Socials] El link debe ser una URL válida (ej. https://...)"),
		})
		.strict(),
});
...
const workExperience = defineCollection({
	...
	schema: z
		.object({
			...
			title: z
				.string()
				.min(2, "[Work] El título debe tener al menos 2 caracteres"),
			company: z.string().min(1, "[Work] La compañía es obligatoria"),
			duration: z
				.string()
				.min(4, "[Work] La duración es obligatoria (ej. '2023 - 2024')"),
			description: z
				.string()
				.min(
					10,
					"[Work] La descripción debe ser detallada (mínimo 10 caracteres)",
				),
		})
		.strict(),
});
		...
		z
			.object({
				...
				createdAt: z.coerce.date({
					required_error: "Falta la fecha de creación en el frontmatter",
				}),
	...
	schema: ({ image }) =>
		z
			.object({
				...
				description: z
					.string()
					.min(10, "[Projects] La descripción del proyecto es muy corta"),
				date: z.coerce.date({
					required_error: "[Projects] Añade la fecha del proyecto",
				}),
				image: image(),
